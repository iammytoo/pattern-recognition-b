...
def main():
    ...
    # imageのフィルター
    train_dataset = train_dataset.filter(lambda x: x['type'] == 'text')
    val_dataset   = val_dataset.filter(lambda x: x['type'] == 'text')
    test_dataset  = test_dataset.filter(lambda x: x['type'] == 'text')
    ...
    # 結果保存
    save_path = "result/regression_text/xgboost.csv"
