"""
encoderによる埋め込みをconcatしたデータに加えて、コサイン類似度特徴量を利用した決定木による分類

1. Dataloaderによるデータの読み込み
2. EmbeddingProcessor (JapaneseClipImageEmbedder) による埋め込み
3. コサイン類似度特徴量の追加
4. PCAによる特徴量圧縮
5. XGBoostClassifierによる分類（スコアを四捨五入して整数化）
"""
...
def save_results_and_visualize(classifier, train_metrics, val_metrics, test_metrics,
    ...
    # 分類結果用ディレクトリを作成（コサイン類似度版）
    classification_result_dir = os.path.join(DATA_DIR_PATH, "classification_with_cos_sim_result")
    ...
    classifier.save_model(os.path.join(classification_result_dir, "xgboost_humor_classifier_with_cos_sim.pkl"))
    ...
    try:
        ...
        classifier.plot_feature_importance(
            feature_names, 
            top_n=20,
            save_path=os.path.join(classification_result_dir, "feature_importance.png")
        )
...
def main():
    ...
    (X_train_combined, y_train, X_val_combined, y_val, X_test_combined, y_test, 
     pca_odai, pca_response) = apply_pca_compression(
    ...
    classifier, train_metrics, val_metrics, test_metrics = train_and_evaluate_model(
        X_train_combined, y_train_class, X_val_combined, y_val_class, X_test_combined, y_test_class, 
    ...
    save_results_and_visualize(
        ...
        pca_odai, pca_response, X_test_combined, y_test_class
    )
