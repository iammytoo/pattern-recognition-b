...
def save_results_and_visualize(classifier, train_metrics, val_metrics, test_metrics, X_test, y_test):
    """
    結果の保存と可視化
    ...
    # 分類結果用ディレクトリを作成（コサイン類似度のみ版）
    classification_result_dir = os.path.join(DATA_DIR_PATH, "classification_only_cos_sim_result")
    os.makedirs(classification_result_dir, exist_ok=True)
    
    # XGBoostモデルを保存
    classifier.save_model(os.path.join(classification_result_dir, "xgboost_humor_classifier_only_cos_sim.pkl"))
    ...
    for i, col in enumerate(X_test.columns):
        importance = classifier.feature_importance[i] if classifier.feature_importance is not None else 0
...
def main():
    ...
    classifier, train_metrics, val_metrics, test_metrics = train_and_evaluate_model(
        X_train, y_train_class, X_val, y_val_class, X_test, y_test_class, 
        optimize_hyperparams=OPTIMIZE_HYPERPARAMS
    )
    
    # 結果保存・可視化
    save_results_and_visualize(
        classifier, train_metrics, val_metrics, test_metrics, X_test, y_test_class
    )
