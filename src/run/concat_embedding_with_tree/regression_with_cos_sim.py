"""
encoderによる埋め込みをconcatしたデータに加えて、コサイン類似度特徴量を利用した決定木による回帰

1. Dataloaderによるデータの読み込み
2. EmbeddingProcessor (JapaneseClipImageEmbedder) による埋め込み
3. コサイン類似度特徴量の追加
4. PCAによる特徴量圧縮
5. XGBoostRegressorによる回帰
"""
...
def add_similarity_features(train_df, validation_df, test_df):
    """
    コサイン類似度特徴量を追加
    ...
    Returns:
        tuple: コサイン類似度特徴量が追加されたDataFrame
    """
    print("\n--- コサイン類似度特徴量の追加 ---")
    ...
    print(f"特徴量追加後の形状:")
    print(f"    Train     : {train_df_with_sim.shape}")
    print(f"    Validation: {validation_df_with_sim.shape}")
    print(f"    Test      : {test_df_with_sim.shape}")
    
    return train_df_with_sim, validation_df_with_sim, test_df_with_sim


def apply_pca_compression(train_df, validation_df, test_df):
    """
    PCA圧縮を適用（コサイン類似度特徴量は圧縮せずそのまま使用）
    
    Args:
        train_df, validation_df, test_df: コサイン類似度特徴量付きDataFrame
    ...
    X_train_combined = pd.concat([
        ...
        X_train_sim.reset_index(drop=True)
    ], axis=1)
    ...
    X_val_combined = pd.concat([
        ...
        X_val_sim.reset_index(drop=True)
    ], axis=1)
    ...
    X_test_combined = pd.concat([
        ...
        X_test_sim.reset_index(drop=True)
    ], axis=1)
    
    print(f"結合後の特徴量次元: {X_train_combined.shape[1]} (PCA: {PCA_COMPONENTS*2}, 類似度: {len(similarity_cols)})")
    
    return (X_train_combined, y_train, X_val_combined, y_val, X_test_combined, y_test, 
...
def save_results_and_visualize(regressor, train_metrics, val_metrics, test_metrics,
                              pca_odai, pca_response, X_test_combined, y_test):
    """
    結果の保存と可視化
    ...
    Args:
        ...
        X_test_combined, y_test: テストデータ
    """
    print("\n--- 結果保存と可視化 ---")
    
    # 回帰結果用ディレクトリを作成（コサイン類似度版）
    regression_result_dir = os.path.join(DATA_DIR_PATH, "regression_with_cos_sim_result")
    os.makedirs(regression_result_dir, exist_ok=True)
    
    # PCAモデルとXGBoostモデルを保存
    with open(os.path.join(regression_result_dir, "pca_odai_model.pkl"), "wb") as f:
        pickle.dump(pca_odai, f)
    
    with open(os.path.join(regression_result_dir, "pca_response_model.pkl"), "wb") as f:
        pickle.dump(pca_response, f)
    
    regressor.save_model(os.path.join(regression_result_dir, "xgboost_humor_regressor_with_cos_sim.pkl"))
...
def main():
    ...
    # コサイン類似度特徴量の追加
    train_df_with_sim, validation_df_with_sim, test_df_with_sim = add_similarity_features(
        train_df, validation_df, test_df
    )
    
    # PCA特徴量圧縮
    (X_train_combined, y_train, X_val_combined, y_val, X_test_combined, y_test, pca_odai, pca_response) = apply_pca_compression(
        train_df_with_sim, validation_df_with_sim, test_df_with_sim
    )
    
    # XGBoostモデル訓練・評価
    regressor, train_metrics, val_metrics, test_metrics = train_and_evaluate_model(
        X_train_combined, y_train, X_val_combined, y_val, X_test_combined, y_test, optimize_hyperparams=OPTIMIZE_HYPERPARAMS
    )
